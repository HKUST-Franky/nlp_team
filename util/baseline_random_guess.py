...
def load_jsonl_file_to_records_(filename):
    ...
    # Calculate the document length independently
...
def get_prob_dist(dict_reps):
    """
    Calculate the probability distribution from a list of dictionary representations.

    Args:
        dict_reps (list of dict): A list where each dictionary contains:
            - 'text_len' (int): The length of the text.
            - 'soft_labels' (list of dict): A list of spans, where each span is a dictionary with:
                - 'start' (int): The starting index of the span.
                - 'end' (int): The ending index of the span.
                - 'prob' (float): The probability associated with the span.

    Returns:
        dict: A dictionary where the keys are indices and the values are the normalized probabilities.
    """
